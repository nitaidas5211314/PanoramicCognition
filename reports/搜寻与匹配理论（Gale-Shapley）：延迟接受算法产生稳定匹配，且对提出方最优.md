# 搜寻与匹配理论（Gale–Shapley） · 全景认知与实践操作手册

> 延迟接受**总能**给出稳定匹配，而且这个匹配对**提出方最优**、对接受方最劣——谁先开口，谁站在稳定匹配格的有利一端。

> 生成于 2026-09-20｜耗时 24 分 34 秒｜主题：搜寻与匹配理论（Gale–Shapley）｜标签：博弈论、匹配理论、Gale-Shapley、延迟接受、稳定匹配、市场设计、NRMP、择校
...
**▶ 学校侧是偏好还是行政优先级？有没有大量平局？**

有，则「策略防操纵」要按择校文献重述，不能照搬婚姻模型的严格序。

**▶ 委员会用的效果量是 3 人例题还是历史名单？**
