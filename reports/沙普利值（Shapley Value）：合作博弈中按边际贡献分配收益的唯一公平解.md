# 沙普利值（Shapley Value） · 全景认知与实践操作手册

> 合作博弈里按**边际贡献**分配收益的唯一公平解——公理钉死公式，排列平均给出直觉；从手套博弈到 SHAP、电力成本分摊。

> 生成于 2026-09-19｜耗时 7 分 45 秒｜主题：沙普利值（Shapley Value）｜标签：博弈论、合作博弈、沙普利值、Shapley、核、SHAP、成本分摊、权力指数
